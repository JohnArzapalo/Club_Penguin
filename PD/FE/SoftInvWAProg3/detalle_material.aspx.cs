...
namespace SoftInvWAProg3
{
    public partial class detalle_material : System.Web.UI.Page
    {
        ...
        {
            materialDTO material = materialCliente.obtenerMaterialPorId(materialId);
        ...
        private void MostrarInformacionBasica(materialDTO material)
        {
        ...
        private void MostrarAtributosEspecificos(materialDTO material)
        {
            string html = string.Empty;

            switch (material.tipoMaterial)
            {
                case tipoMaterial.LIBRO:
            ...
            if (bibliotecas != null && bibliotecas.Length > 0)
            {
                ...
                foreach (bibliotecaDTO b in bibliotecas)
                {
                    ...
                    if (!bibliotecaSeleccionadaId.HasValue || b.bibliotecaId == bibliotecaSeleccionadaId.Value)
                    {
                        ...
                        ejemplarDTO[] ejemplares = ejemplarCliente.obtenerEjemplaresMaterialPorBiblioteca(materialId, b.bibliotecaId);
        ...
        private List<object> CargarEjemplares(ejemplarDTO[] ejemplares)
        {
            List<object> resultado = new List<object>();

            foreach (ejemplarDTO e in ejemplares)
            {
                resultado.Add(new
                {
                    Codigo = e.ejemplarId,
        ...
        protected string ObtenerEstado(estadoEjemplar estado)
        {
            switch (estado)
            {
                case estadoEjemplar.PRESTADO:
                    return "<span class='text-primary fw-semibold'>Prestado</span>";
                case estadoEjemplar.EN_REPARACION:
